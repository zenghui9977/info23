...
class BaseFL(object):
    ...
    def init_fl_dataset(self, fl_dataset=None):
        ...
        if fl_dataset is not None:
            self.fl_dataset = fl_dataset(data_name=self.config.dataset.data_name,
                                            trainset=trainset,
                                            testset=testset,
    ...
    def init_server(self, current_round=0, custome_server=None):

        if custome_server is None:
            ...
            logger.info('Custome Server initialization.')
            self.server = custome_server(config=self.config, 
                                    clients=self.clients_id, 
                                    client_class=self.client_class,
                                    global_model=self.global_model,
                                    fl_trainset=self.fl_dataset,
    ...
    def init_clients(self, custome_client_class=None):

        if custome_client_class is None:
            ...
            logger.info('Custome Clients initialization.')
            self.client_class = custome_client_class(config=self.config, device=self.device)
    ...
    def init_fl(self, config=None, fl_dataset=None, global_model=None, custome_server=None, custome_client_class=None, exp_name=None, current_round=0):
        ...
        self.init_clients(custome_client_class=custome_client_class)

        if self.config.is_visualization:
            self.init_visualization()

        self.init_server(current_round=current_round, custome_server=custome_server)
...
def init(config=None, custome_client_class=None, custome_server=None):
    ...
    global_fl.init_fl(config, custome_server=custome_server, custome_client_class=custome_client_class)
